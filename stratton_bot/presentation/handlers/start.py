...
async def cmd_start(message: Message, menu_use_case: MenuUseCase, localizer: Localizer) -> None:
    user = message.from_user
    mention = f"@{user.username}" if user and user.username else (user.full_name if user else "user")
    _, active_testing = await menu_use_case.register_user(
        user_id=user.id,
    ...
    await message.answer(
        ...
        reply_markup=main_menu(localizer, has_testing=bool(active_testing)),
    )


@router.callback_query(F.data == "main_menu")
async def cb_main(callback: CallbackQuery, menu_use_case: MenuUseCase, localizer: Localizer) -> None:
    _, active_testing = await menu_use_case.register_user(
